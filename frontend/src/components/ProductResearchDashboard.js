...
function ProductResearchDashboard({ 
  ...
  const [darkMode, setDarkMode] = useState(() => {
    const saved = localStorage.getItem('product-research-dark-mode');
  ...
  const toggleDarkMode = () => {
    ...
    setDarkMode(newMode);
    localStorage.setItem('product-research-dark-mode', JSON.stringify(newMode));
  };
  ...
  const fetchDepartmentIssues = async () => {
    try {
      const response = await axios.get(`${API_URL}/issues/department/product-research`);
  ...
  const departmentEmployees = employees.filter(emp => emp.department === 'product-research');
  
  // Filter tasks assigned to department employees
  const departmentTasks = tasks.filter(task => {
    const employee = employees.find(emp => emp.id === task.employeeId);
    return employee && employee.department === 'product-research';
  });
  ...
  const departmentAttendance = attendance.filter(att => {
    ...
    return employee && employee.department === 'product-research' && att.date === selectedDate;
  });
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={{ 
        ...
        <div style={cardStyle}>
          <h3 style={{ margin: '0 0 15px 0', color: '#f39c12' }}>📋 Research Tasks</h3>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{pendingTasks}</div>
              <div style={{ color: '#f39c12' }}>Ongoing</div>
            </div>
            <div>
        ...
        <div style={cardStyle}>
          <h3 style={{ margin: '0 0 15px 0', color: '#9b59b6' }}>🧪 Research Team</h3>
          <div style={{ fontSize: '32px', fontWeight: 'bold', marginBottom: '10px' }}>
            {totalEmployees}
          </div>
      ...
      <div style={cardStyle}>
        <h3 style={{ marginBottom: '20px', color: '#e74c3c' }}>🚨 Research Issues</h3>
        {loading ? (
          <p>Loading issues...</p>
        ) : departmentIssues.length === 0 ? (
          <p style={{ color: '#7f8c8d', fontStyle: 'italic' }}>No issues assigned to product research department</p>
        ) : (
          <div style={{ display: 'grid', gap: '15px' }}>
            {departmentIssues.map(issue => (
              ...
              >
                ...
                {/* Action buttons for research team */}
                {issue.status !== 'Closed' && (
      ...
      {/* Research Metrics */}
      <div style={cardStyle}>
        <h3 style={{ marginBottom: '20px', color: '#9b59b6' }}>📈 Research Metrics</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ opacity: 0.7 }}>Research Completed This Week</div>
          </div>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ opacity: 0.7 }}>Overdue Research</div>
          </div>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ opacity: 0.7 }}>Research Success Rate</div>
          </div>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ opacity: 0.7 }}>Issues Solved This Week</div>
          </div>
        </div>
      </div>
      ...
      {/* Department Research Tasks */}
      <div style={cardStyle}>
        <h3 style={{ marginBottom: '20px', color: '#f39c12' }}>🔬 Current Research Tasks</h3>
        {departmentTasks.length === 0 ? (
          <p style={{ color: '#7f8c8d', fontStyle: 'italic' }}>No research tasks assigned</p>
        ) : (
          <div style={{ display: 'grid', gap: '10px' }}>
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '14px', opacity: 0.7 }}>
                    Researcher: {getEmployeeName(task.employeeId)} | 
      ...
      {/* Research Categories */}
      <div style={cardStyle}>
        <h3 style={{ marginBottom: '20px', color: '#2ecc71' }}>📚 Research Focus Areas</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
          <div style={{ 
            ...
            <h4 style={{ margin: '0 0 10px 0', color: '#3498db' }}>🛍️ Market Research</h4>
          ...
          <div style={{ 
            ...
            <h4 style={{ margin: '0 0 10px 0', color: '#f39c12' }}>📈 Product Analytics</h4>
            <p style={{ margin: 0, fontSize: '14px', opacity: 0.7 }}>
          ...
          <div style={{ 
            ...
            <h4 style={{ margin: '0 0 10px 0', color: '#e74c3c' }}>🔍 Quality Assessment</h4>
            <p style={{ margin: 0, fontSize: '14px', opacity: 0.7 }}>
              Product quality evaluation and improvement suggestions
            </p>
          </div>
          ...
          <div style={{ 
            ...
            <h4 style={{ margin: '0 0 10px 0', color: '#9b59b6' }}>💡 Innovation Research</h4>
            <p style={{ margin: 0, fontSize: '14px', opacity: 0.7 }}>
              Exploring new product opportunities and technologies
            </p>
          </div>
        </div>
      </div>
...
export default ProductResearchDashboard;