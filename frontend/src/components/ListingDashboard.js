...
function ListingDashboard({ 
  ...
  const [darkMode, setDarkMode] = useState(() => {
    const saved = localStorage.getItem('listing-dark-mode');
  ...
  const toggleDarkMode = () => {
    ...
    setDarkMode(newMode);
    localStorage.setItem('listing-dark-mode', JSON.stringify(newMode));
  };
  ...
  const fetchDepartmentIssues = async () => {
    try {
      const response = await axios.get(`${API_URL}/issues/department/listing`);
  ...
  const departmentEmployees = employees.filter(emp => emp.department === 'listing');
  
  // Filter tasks assigned to department employees
  const departmentTasks = tasks.filter(task => {
    const employee = employees.find(emp => emp.id === task.employeeId);
    return employee && employee.department === 'listing';
  });
  ...
  const departmentAttendance = attendance.filter(att => {
    ...
    return employee && employee.department === 'listing' && att.date === selectedDate;
  });
    ...
    <div style={containerStyle}>
      ...
      <div style={cardStyle}>
        ...
        {loading ? (
          ...
          <p style={{ color: '#7f8c8d', fontStyle: 'italic' }}>No issues assigned to listing department</p>
        ) : (
